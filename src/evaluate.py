...
def metrics(dataset, y_true, sample_size, runs=100):
    precision = np.zeros((runs, 3))
    recall = np.zeros((runs, 3))
    f_score = np.zeros((runs, 3))
    for run in range(0, runs):
        sampled_dataset = sample(dataset, sample_size)
